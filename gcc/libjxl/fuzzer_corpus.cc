...
//    jxl::ImageBundle ib(&io.metadata.m);
    const bool has_alpha = false; //spec.alpha_bit_depth != 0;
    const size_t bytes_per_sample = 1;
//        jxl::DivCeil(32 /*io.metadata.m.bit_depth.bits_per_sample*/, 8);
    const size_t bytes_per_pixel =
        bytes_per_sample *
        (3/*io.metadata.m.color_encoding.Channels()*/ + has_alpha);
